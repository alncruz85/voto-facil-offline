...
// ==========================================
// ERROR BOUNDARY GLOBAL (Anti-White Screen)
// ==========================================
...
class ErrorBoundary extends Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    console.error("[ErrorBoundary] Erro crítico capturado na UI:", error, errorInfo);
  }
    ...
    if (this.state.hasError) {
      return (
        <div className="min-h-screen bg-slate-900 text-white flex flex-col items-center justify-center p-6 text-center">
          <div className="w-16 h-16 bg-red-600/20 text-red-500 rounded-2xl flex items-center justify-center text-3xl mb-4 border border-red-500/30">
            ⚠️
          </div>
          <h2 className="text-xl font-bold mb-2">Ops! Ocorreu um erro no aplicativo</h2>
          <p className="text-xs text-slate-400 max-w-md mb-6 font-mono bg-slate-800 p-3 rounded-xl border border-slate-700 text-left overflow-auto">
          ...
          <button
            ...
            className="inline-flex items-center px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl shadow transition-all text-sm"
          >
            <RefreshCw className="w-4 h-4 mr-2" /> Reiniciar e Limpar Dados
          </button>
        </div>
      );
    }
...
export function App() {
  ...
  useEffect(() => {
    console.log("[App] Montado. Carregando listas do repositório...");
    try {
      const carregadas = repository.listar();
      setListas(carregadas);
      console.log(`[App] ${carregadas.length} listas carregadas com sucesso.`);
    } catch (err: any) {
      console.error("[App] Erro ao carregar listas iniciais:", err);
    ...
    const atualizar = () => {
      try {
        const atualizadas = repository.listar();
        setListas(atualizadas);
        console.log(`[App] Estado sincronizado. Total de listas: ${atualizadas.length}`);
      } catch (err: any) {
        console.error("[App] Erro ao sincronizar listas:", err);
      }
    };
  ...
  if (carregando) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 text-center">
        ...
        <h2 className="text-base font-bold text-slate-800">Carregando Voto Fácil...</h2>
        <p className="text-xs text-slate-500 mt-1">Preparando ambiente seguro offline.</p>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col font-sans">
        ...
        <header className="bg-blue-900 text-white shadow-md">
          <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
            <div
              ...
              onClick={() => {
                console.log("[App] Navegando para Menu 1 (listas)");
                setView({ name: "listas" });
              }}
            >
        ...
        {erroGlobal && (
          <div className="bg-red-50 border-b border-red-200 p-3 text-center text-red-800 text-sm flex items-center justify-center space-x-2">
            ...
            <button onClick={() => setErroGlobal(null)} className="underline ml-4 font-semibold">
              Fechar
            </button>
          </div>
        )}
        ...
        <main className="flex-1 max-w-4xl w-full mx-auto p-4 sm:p-6">
          {view.name === "listas" && (
            <ListaView
              listas={listas}
              onSelectLista={(id) => {
                console.log(`[App] Abrindo detalhes da lista ID: ${id}`);
                setView({ name: "detalhes", listaId: id });
              }}
              onSimularUrna={(id) => {
                console.log(`[App] Iniciando simulador de urna para ID: ${id}`);
                setView({ name: "urna", listaId: id });
              }}
              onCompartilhar={(id) => {
                console.log(`[App] Abrindo QR Code para ID: ${id}`);
                setView({ name: "qr", listaId: id });
              }}
...
function UrnaView({
  ...
  const [_, setSomBeep] = useState(false);
